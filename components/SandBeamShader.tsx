...
void main() {
  ...
  // Single diagonal beam (no cross): all ribbons share one positive slope.
  float broadA = ribbon(wuv, 0.02 + 0.05 * sin(t * 0.33), 0.82, 0.018, t);
  float broadB = ribbon(wuv, 0.16 + 0.04 * cos(t * 0.27), 0.82, 0.030, -t * 0.72);
  float sharp = ribbon(wuv, 0.09 + 0.04 * sin(t * 0.2), 0.82, 0.0018, t * 1.45);

  float beams = clamp(broadA * 0.95 + broadB * 0.78 + sharp * 1.5, 0.0, 1.0);
  ...
  // Pink hues: soft rose base warming into a restrained magenta beam, blush highlights.
  vec3 rose = vec3(0.980, 0.560, 0.760);
  vec3 deepRose = vec3(0.910, 0.330, 0.620);
  vec3 blush = vec3(1.000, 0.880, 0.935);
  vec3 magenta = vec3(0.886, 0.000, 0.455);
  vec3 white = vec3(1.000);

  vec3 color = mix(rose, deepRose, smoothstep(0.05, 0.95, warp) * 0.34);
  color = mix(color, blush, edge * 0.42);
  // Magenta only in the brightest beam core — kept minimal.
  color = mix(color, magenta, core * 0.32);
  color = mix(color, blush, core * 0.62);

  float speckles = (sand - 0.5) * (0.20 + edge * 0.62);
  color += speckles;

  float whiteDust = step(0.66, sand) * edge * 0.48;
  float roseDust = step(0.83, salt) * (1.0 - core) * 0.14;
  color = mix(color, white, whiteDust);
  color = mix(color, deepRose, roseDust);
